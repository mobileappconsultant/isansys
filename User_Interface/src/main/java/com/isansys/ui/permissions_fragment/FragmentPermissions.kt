...
binding.tableWriteExternalStoragePermission.visibility = View.GONE
}
...
// Attach observer
    ...
    override fun onRequestPermissionsResult(
        ...
        when (requestCode) {
            PermissionsImpl.CAMERA_PERMISSION_REQUEST_CODE -> {
                viewModel.isHaveCamera()
            }
            PermissionsImpl.REQUEST_AUDIO_CODE -> {
                viewModel.isHaveRecordAudio()
            }
            PermissionsImpl.WRITE_EXTERNAL_STORAGE_PERMISSION_REQUEST_CODE -> {
                viewModel.isHaveReadWrite()
            }
        }
    }
