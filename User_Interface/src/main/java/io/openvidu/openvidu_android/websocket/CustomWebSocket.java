...
public class CustomWebSocket extends AsyncTask<MainActivity, Void, Void> implements WebSocketListener {
    ...
    private final AtomicInteger ID_JOINROOM = new AtomicInteger(-1);
    private final AtomicInteger ID_LEAVEROOM = new AtomicInteger(-1);
    private final AtomicInteger ID_PUBLISHVIDEO = new AtomicInteger(-1);
    private final Map<Integer, String> IDS_RECEIVEVIDEO = new ConcurrentHashMap<>();
    private final Set<Integer> IDS_ONICECANDIDATE = Collections.newSetFromMap(new ConcurrentHashMap<>());
