...
//        return otherTask.getPriority().ordinal() >= PE_TaskPriority.FOR_EXPLICIT_BONDING_AND_CONNECTING.ordinal();
    ...
    protected void selfInterrupt()
    {
        ...
        double time_remaining = m_scanTime - getTotalTimeExecuting(getManager().currentTime());

        if( wasExecuting )
        {
            if (time_remaining >= getMinimumScanTime())
            {
                timeout();
            }
            else
            {
                m_scanTime = time_remaining;
                super.selfInterrupt();
            }
        }
    }
