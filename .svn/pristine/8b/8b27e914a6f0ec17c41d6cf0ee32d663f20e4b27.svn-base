...
public class GridLabelRenderer {
    ...
    /* humanRound defaults to rounding to the nearest integer, can be modified using setRoundingValue */
    private double mRoundHorizontalToNearest = 1;
    ...
    protected boolean adjustHorizontal(boolean changeBounds) {
        ...
        // human rounding to have nice numbers (1, 2, 5, ...)
        if (isHumanRoundingX()) {
            exactSteps = humanRound(exactSteps, mRoundHorizontalToNearest);
    ...
    public void setHorizontalRoundingValue(double roundToNearest) {
        mRoundHorizontalToNearest = roundToNearest;
    }


    protected double humanRound(double in, double roundToNearest) {
        double temp = in/roundToNearest;

        if(temp < 0.5)
        {
            temp = 0.5; // Distance between labels should not be 0 as otherwise we'll have an infinite number of them and the graph code will hang. 0.5 here will round up instead of down.
        }

        return roundToNearest*Math.floor(temp + 0.5);
    }
