...
import com.isansys.patientgateway.algorithms.DataPoint;
...
public abstract class IIRFilter 
{
	 ...
	 * @param sampleRate
	 *            the sample rate of signal to be filtered
	 */
	...
	public boolean process(float[] float_buffer) 
	{
	...
	public boolean process(List<DataPoint> data)
	{
		...
		for (int i = 0; i < length; i++) 
		{
			//shift the in array
			System.arraycopy(in, 0, in, 1, in.length - 1);
			in[0] = (float) data.get(i).getAmplitude();
			...
			data.get(i).setAmplitude(y); // ToDo: does making this an int cause accuracy to suffer at all?
		}
