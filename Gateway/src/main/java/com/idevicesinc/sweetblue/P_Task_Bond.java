...
final class P_Task_Bond extends PA_Task_RequiresBleOn
{
    ...
    @Override public final void execute()
    {
        ...
        //cache the bond state here, in case it changes between polling
        int bondState = getDevice().m_nativeWrapper.getNativeBondState();
        getLogger().d("Bond state = " + getBondStateString(bondState));

        if (getDevice().m_nativeWrapper.isNativelyBonded(bondState))
        {
        ...
        {
            if (getDevice().m_nativeWrapper./*already*/isNativelyBonding(bondState))
            {
                // nothing to do
                getLogger().w("Already bonding!");
            }
            else if (false == m_explicit)
            {
                if (getDevice().m_nativeWrapper./*already*/isNativelyUnbonded())
                {
                    getLogger().e("Unexpected Unbonding in implicit bond!!!!");
                ...
                {
                    // nothing to do???
                    getLogger().w("Unknown Bond State!!! Wait for next update...");
                }
            }
    ...
    private String getBondStateString(int bondState)
    {
        ...
        switch(bondState)
        {
            ...
            default:
            {
                state_string = "UNKNOWN : " + bondState;
            }
        }
