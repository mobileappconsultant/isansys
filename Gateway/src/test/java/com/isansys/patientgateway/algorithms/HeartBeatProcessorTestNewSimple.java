 ...
 */
public class HeartBeatProcessorTestNewSimple
{
    @Mock
    ...
    public void setUp()
    {
        ...
        when(dummy_settings.getSimpleHeartRateEnabledStatus()).thenReturn(true);
    ...
    public void HeartBeatProcessorTest_fakeDataThreeMinutesWithBigRR() throws Exception
    {
        ...
        /* Second values calculated - the second minute - four beats missing
         * check HR values - type = 0, HR = 56 and timestamp is 180000.
         */
        assertEquals(0, (int) integers.get(4));
        assertEquals(56, (int) integers.get(5));
        assertEquals(180000, (long) timestamps.get(2));
        ...
        /* Third values calculated - the third minute - 11 beats missing
         * check HR values - type = 0, HR = 60 and timestamp is 240000.
         */
        assertEquals(0, (int) integers.get(8));
        assertEquals(49, (int) integers.get(9));
        assertEquals(240000, (long) timestamps.get(4));
    ...
    public void HeartBeatProcessorTest_fakeDataThreeMinutesWithOrientationBeats() throws Exception
    {
         ...
         * The first one is never reported, as it has no RR interval, so altogether 181 are reported and all of them have valid RR intervals.
         *
         * NOTE that no orientation beats are reported, as they use a different tag and are not sorted with tho other beats.
         */
    ...
    public void HeartBeatProcessorTest_fakeDataThreeMinutesMiscErrorCodes() throws Exception
    {
        ...
        for(int i = 0; i <= 180; i++)
        {
            ...
            {
                // uses a separate "Debug heartbeat" tag number
...
//        }
    ...
    public void HeartBeatProcessorTest_threeHeartBeatsProcessedNormally() throws Exception
    {
        ...
        /* Check values calculated - the first minute
         * HR should be 3 BPM, as the simple HR just counts the number of beats.
         */
        assertEquals(0, (int) integers.get(0));
        assertEquals(3, (int) integers.get(1));
        assertEquals(60000, (long) timestamps.get(0));

        // RR is zero - with an HR of 3, we'll get a sampling rate of 1 (the minimum allowed).
        // and a low pass cutoff of < 4 breaths per minute (in fact, 1.2). This is the case for HR <= 10
        // meaning we're filtering out all frequencies below 4 BR/min and above 4 BR/min - i.e. all of them.
        // so we end up with (more or less) a flat line signal, with no respiration features in it.
        assertEquals(1, (int) integers.get(2));
        assertEquals(0, (int) integers.get(3));
        assertEquals(60000, (long) timestamps.get(1));
    }
